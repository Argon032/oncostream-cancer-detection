...
swin_predict.py
---------------
Inference script for the Swin Transformer model.
Handles both brain (4 classes) and breast (2 classes) datasets.

Used by:
    - api/main.py  (called per uploaded image)
    - standalone use for quick testing

Usage (standalone):
    from inference.swin_predict import predict
...
def load_model(dataset: str, project_root: str = "."):
    ...
    if not os.path.exists(ckpt_path):
        raise FileNotFoundError(
            ...
            f"Run train_transformer.py with model_name='swin' and dataset='{dataset}' first."
        )
...
def predict(image, dataset: str, project_root: str = "."):
    """
    Run Swin inference on a single image.

    Args:
        image:        PIL Image or path string to the image file.
        dataset:      'brain' or 'breast'
        project_root: Root folder of the project. On Colab:
                      "/content/drive/MyDrive/oncostream-cancer-detection"
