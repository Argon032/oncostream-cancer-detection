...
--------------------
Inference script for the MobileNetV2 model.
...
def load_model(dataset: str, project_root: str = "."):
    ...
    ckpt_path = os.path.join(project_root, "results", dataset, "mobilenet_best.pth")
    if not os.path.exists(ckpt_path):
        raise FileNotFoundError(
            f"Checkpoint not found at {ckpt_path}. "
            f"Run train_cnn.py with model_name='mobilenet' and dataset='{dataset}' first."
        )
...
if __name__ == "__main__":
    ...
    if img_path is None:
        print("Usage: python mobilenet_predict.py <image_path> <brain|breast>")
        sys.exit(1)
