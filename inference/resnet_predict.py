...
resnet_predict.py
-----------------
Inference script for the ResNet-50 model.
Handles both brain (4 classes) and breast (2 classes) datasets.

Usage (standalone):
    from inference.resnet_predict import predict
...
def load_model(dataset: str, project_root: str = "."):
    ...
    ckpt_path = os.path.join(project_root, "results", dataset, "resnet50_best.pth")
    if not os.path.exists(ckpt_path):
        raise FileNotFoundError(
            f"Checkpoint not found at {ckpt_path}. "
            f"Run train_cnn.py with model_name='resnet50' and dataset='{dataset}' first."
        )
...
def predict(image, dataset: str, project_root: str = "."):
    """
    Run ResNet-50 inference on a single image.
...
if __name__ == "__main__":
    ...
    if img_path is None:
        print("Usage: python resnet_predict.py <image_path> <brain|breast>")
        sys.exit(1)
